...
import { X, Lock, CheckCircle2, AlertTriangle, ShieldCheck, ArrowRight, Truck } from 'lucide-react';
...
export const PayPalCheckoutModal: React.FC<PayPalCheckoutModalProps> = ({
  ...
  const handlePayPalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success - client bought and paid for product
      setIsProcessing(false);

      // Instantly launch the WhatsApp alert to the merchant's number with all 6 details
      if (data.whatsappUrl) {
        try {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-fade-in">
      <div className="bg-[#13151f] text-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-[#d4a853]/40 my-auto">
        ...
        {/* Checkout Form */}
        <form onSubmit={handlePayPalSubmit} className="p-5 sm:p-6 space-y-4 text-xs">
          {errorMessage && (
          ...
          <button
            ...
            className="w-full py-4 px-6 rounded-2xl bg-[#ffc439] hover:bg-[#ffb000] disabled:opacity-50 text-[#003087] font-black text-sm tracking-wide shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer hover:scale-101 active:scale-99"
          >
            {isProcessing ? (
              ...
              <>
                <span>Complete Purchase with PayPal UK</span>
                <span>&bull;</span>
